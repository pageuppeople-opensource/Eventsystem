        ...
        public static async Task<List<Subscription>> GetSubscriptions(AWSCredentials awsCredentials = null)
        {
            ...
            using (var client = CreateClient(awsCredentials))
            {
                try
                {
                    ...
                    if (modifiedAt > _lastModified)
                    {
                        var content = await GetContent(client);
                        _subscriptions = JsonConvert.DeserializeObject<List<Subscription>>(content);
            ...
            Console.WriteLine($"GetSubscription: Found {_subscriptions.Count} subscriptions");
